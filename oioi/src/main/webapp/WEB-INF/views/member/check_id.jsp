...
<script type="text/javascript">
	...
	$(function() {
	    ...
	    $("#btnCheckId").click(function() {
	    	...
	    	// 아이디 중복 확인을 위해 서버에 AJAX 요청
	    	$.ajax({
                type: "POST",
                url: "check_id", // 서버측 URL
	    ...
	    $("#btnUseID").click(function() { // 아이디 사용 버튼 클릭 이벤트
	        if(checkIdResult) {
	            window.opener.document.fr.member_id.value = $("#member_id").val();
	            window.close();
	        } else {
		...
		<div>
			<c:choose>
				<c:when test="${param.isValid eq true}">
					<input type="button" class="id_check" value="아이디 사용" id="btnUseID"><br>
				</c:when>
				<c:otherwise>
					<input type="button" class="id_check" value="중복 확인" id="btnCheckId">
				</c:otherwise>
			</c:choose>
		</div>
	</form>
